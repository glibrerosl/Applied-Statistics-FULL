...
La navegación a través de las pestañas muestra que la reducción de la dimensionalidad del conjunto de datos conduce a calidades de representación altas en términos de la escala de contribuciones relativas basada en un cociente de proyecciones con propiedades aditivas y de respuesta en escala continua entre $0$ y $1$. Así, en particular:

El **Círculo de Correlaciones**...

La **Matriz de Representación**...

La **Calidad de Representación**

Las **Coordenadas Individuales**...
...
#### Coordenadas Individuales [Subconjunto]
```{r Coordenadas_Individuales_Subconjunto, fig.align = 'center'}
set.seed(780728)
data_50_UR <- Admission_Dataset_Initial[sample(1:nrow(Admission_Dataset_Initial),50),-c(1,2,8)]
set.seed(780728)
data_50 <- cbind(Admission_Dataset_Initial[sample(1:nrow(Admission_Dataset_Initial),50),-c(1,2,8,9)], data_50_UR$University_Rating)
head(PCA(data_50, ncp = 6, scale.unit = T, graph = F, quali.sup = 7)$ind$coord, n = 50L)
```
...
set.seed(780728)
data_50 <- cbind(Admission_Dataset_Initial[sample(1:nrow(Admission_Dataset_Initial),50),-c(1,2,8,9)], data_50_UR$University_Rating)
fviz_pca_biplot(PCA(data_50, ncp = 6, scale.unit = T, graph = F, quali.sup = 7), axes = c(1, 2), repel = T, habillage = 7)
```
